...
- **`src/layouts/Base.astro`** wraps every page: it owns the `<head>` (SEO meta,
  canonical, Open Graph/Twitter tags, site-level JSON-LD, fonts) plus the shared
  `<Header>` / `<Footer>` and the reveal-on-scroll script. Pages pass `title`,
  optional `description`, `ogTitle`/`ogDescription` overrides, and `noindex`.
  Extra JSON-LD lives where it's page-specific: `Recipe` schema in
...
- `FREE_UNTIL_ISO` / `FREE_UNTIL_LABEL` — free-access window: ISO date feeds the
  `<time datetime>`, label is the visible text. `isFreeWindowOpen(now?)` gates
  the "gratis por tiempo limitado" notice (compares by Colombia-time date); a
  past date hides the notice automatically.
- `PRICING` — plans that take effect after the free window (`mensual` / `anual`
  in COP + approx USD, `ahorroPct` on annual, plus the `beneficios` list).
  Edit these numbers and the Pricing section updates itself; keep `beneficios`
  free of medical claims and tech-stack names.
